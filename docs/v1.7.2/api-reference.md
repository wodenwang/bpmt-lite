# bpmt-lite v1.7.2 API 参考

本文是 `bpmt-lite` v1.7.2 的 API 归档版文档，面向人类阅读。机器可读的 OpenAPI 快照见 [openapi.json](openapi.json)。
...
v1.7.2 在 v1.7.1 报表视图配置 API 基础上修复 GitHub issue 反馈的运行问题，不新增 API 路径或认证机制。

本版本修复：

- 报表视图创建时 `validate`、`dryRun` 通过但实际写入 `VW_REPORT.SORT` 为空导致 500。
- 动态表视图和报表视图配置写入后未清理运行时 Hibernate 二级缓存与查询缓存。
- 数据库操作 `save` 在显式主键 insert 且数据库不返回自增 key 时误报 500。
- 控制面板注册信息页面未显示 bpmt-lite Maven/WAR 实际版本。
...
- 版本归档 OpenAPI：[docs/v1.7.2/openapi.json](openapi.json)
