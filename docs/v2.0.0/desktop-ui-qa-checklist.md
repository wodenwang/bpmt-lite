...
`v1.9.0` 不只做壳层视觉整理，还必须修复当前已确认的全局前端缺陷。以下问题进入版本完成门禁，不能作为后续观察项跳过。

| 问题 | 风险 | v1.9.0 要求 |
| --- | --- | --- |
| 弹框时部分按钮层级高于遮罩或窗体 | 用户能看到或误点弹窗背后的按钮，破坏 modal 语义 | 建立全局 z-index 层级，确保 overlay 和 dialog 覆盖普通按钮、顶部按钮、列表行操作、zTree 和旧按钮条 |
| 弹窗右上角关闭按钮大小不一 | 弹窗组件不稳定，点击目标不可预测 | 统一 `.ui-dialog-titlebar-close` 宽高、图标居中、hover/focus 和 button 初始化后的最终尺寸 |
| 默认无结果后查询有结果时，空状态和结果表格同时出现 | 用户误判查询结果，页面状态自相矛盾 | 修复通用空状态刷新逻辑；Ajax 更新、查询、分页、排序后重新判断，有数据时移除或隐藏空状态 |
| logo 区、菜单区、顶部功能按钮区、主内容区高度宽度不对齐 | 桌面后台壳层割裂，当前位置和工作区边界不清楚 | 重新设计四区栅格和视觉归属，使 logo+菜单形成导航侧栏，顶部按钮+主内容形成工作区 |
| 非菜单 zTree hover 尺寸变形 | 权限树、组织树、分类树等操作时抖动，影响点击稳定性 | 将 zTree 尺寸稳定规则扩展到全站 `.ztree`，默认、hover、selected、focus 不改变宽高 |

v1.9.0 必验路径：

- 登录后首页：检查 logo 区、菜单区、顶部功能按钮区、主内容区对齐。
- 功能设置模块：检查当前模块、当前菜单、主内容区边界。
- 第三方系统列表：打开 AI 接入提示词弹窗，确认遮罩和窗体覆盖页面按钮，关闭按钮尺寸稳定。
- 一个 `Ui.alert` / `Ui.confirm` 场景：确认右上角关闭按钮、遮罩层级和按钮层级。
- 一个默认空列表：查询出有结果后不再显示“没有匹配的数据”，再次查询无结果时空状态恢复。
- 一个非菜单 zTree 页面：检查 hover、selected、focus 前后节点宽高不变。
