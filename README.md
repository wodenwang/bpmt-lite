...
```bash
curl -fsSL https://github.com/wodenwang/bpmt-lite/raw/refs/tags/v1.7.2/scripts/install.sh | BPMT_HTTPS_ENABLED=1 bash
```
...
`upgrade.sh` 会根据 GitHub 最新 release/tag 下载参考 compose 文件，例如 `docker-compose-v1.7.2.yml`；不会覆盖当前 `docker-compose.yml`。升级时只拉取 BPMT Web/API 的 `latest` 镜像并重启这两个服务，不自动升级 `mariadb`、`nginx` 等第三方容器。升级状态记录在 `.bpmt-lite/` 下，不写入业务数据库。
...
- 文档归档：[v1.7.2 API 归档](docs/v1.7.2/api-reference.md)，[v1.7.2 OpenAPI](docs/v1.7.2/openapi.json)。
...
- [v1.7.2 API 归档](docs/v1.7.2/api-reference.md)：当前 API 的 Markdown 归档，完整接口以 OpenAPI 快照为准。
- [v1.7.2 OpenAPI](docs/v1.7.2/openapi.json)：给 AI agent、N8N、飞书集成平台使用的接口契约。
- [v1.7.2 发布说明](docs/release-v1.7.2.md)：GitHub issue bugfix 版本的修复范围和验证清单。
